...
This prompt works standalone, but is **enhanced** when running with oh-my-codex (OMX) or oh-my-claudecode (OMC). Use these capabilities when available; fall back gracefully when they are not.

### Parallel Delegation (if spawn_agent or Task tool available)

For stories with independent sub-tasks, delegate work in parallel:

**With OMX (Codex CLI -- spawn_agent):**
```
spawn_agent(prompt="<content from /prompts:executor>\n\nImplement the API endpoint for...")
spawn_agent(prompt="<content from /prompts:test-engineer>\n\nWrite tests for...")
```

**With OMC (Claude Code -- Task tool):**
```
...
- `test-engineer` -- test strategy, test writing, coverage
- `designer` -- UI/UX components, styling
- `explore` -- codebase search, file discovery, quick lookups
...
Prefer MCP tools over shell commands when available (same tools across OMC and OMX):
...
- Rely on `progress.txt` and AGENTS.md for cross-iteration memory
...
Additionally, if `project_memory_add_note` is available, persist important patterns there too.
...
4. If reference images are provided in the prompt, analyze them against current screenshots and check for visual regressions
5. Record image-analysis pass/fail observations in `progress.txt`
6. Take a screenshot if helpful for the progress log
...
- Use parallel execution for independent sub-tasks when delegation tools are available
