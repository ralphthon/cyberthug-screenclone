...
class Live2dModel:
    ...
    def set_model(self, model_name: str) -> None:
        ...
        self.emo_map: dict[str, int | list[int]] = {
        ...
        while i < len(str_to_check):
            ...
            for key in self.emo_map.keys():
                ...
                if str_to_check[i : i + len(emo_tag)] == emo_tag:
                    mapped_value = self.emo_map[key]
                    if isinstance(mapped_value, list):
                        if mapped_value:
                            expression_list.append(random.choice(mapped_value))
                    else:
                        expression_list.append(mapped_value)
                    i += len(emo_tag) - 1
                    break
            i += 1
