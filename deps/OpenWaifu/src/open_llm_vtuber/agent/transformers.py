            ...
            *args, **kwargs
            ...
            async for item in stream:
                if (
                    ...
                    elongation_count = min(3, len(re.findall(r"~", sentence.text)))
                    question_count = min(3, len(re.findall(r"[?？]", sentence.text)))
                    exclaim_count = min(3, len(re.findall(r"[!！]", sentence.text)))
                    linebreak_count = min(3, len(re.findall(r"\n+", sentence.text)))
                    if any(tag.name == "think" for tag in sentence.tags):
                        ...
                        if tts:
                            ...
                            if elongation_count > 0:
                                marker_parts.append(
                                    f"<<prosody:elong:{elongation_count}>>"
                                )
                            if question_count > 0:
                                marker_parts.append(
                                    f"<<prosody:question:{question_count}>>"
                                )
                            if exclaim_count > 0:
                                marker_parts.append(
                                    f"<<prosody:exclaim:{exclaim_count}>>"
                                )
                            if linebreak_count > 0:
                                marker_parts.append(
                                    f"<<prosody:linebreak:{linebreak_count}>>"
                                )
                            if marker_parts:
                                tts = f"{' '.join(marker_parts)} {tts}"
