...
class WebSocketHandler:
    ...
    async def _send_neutral_expression_if_available(
        self, websocket: WebSocket, context: ServiceContext
    ) -> None:
        """Send neutral expression once, if model has a neutral mapping."""
        try:
            emotion_map = context.live2d_model.model_info.get("emotionMap", {})
            ...
            logger.warning(f"Failed to send neutral bootstrap expression: {e}")
    ...
    async def _send_initial_messages(
        ...
        await self._send_neutral_expression_if_available(
            websocket, session_service_context
        )
    ...
    async def _handle_config_switch(
        ...
        if config_file_name:
            ...
            await self._send_neutral_expression_if_available(websocket, context)
