...
class Qwen3TTSConfig(I18nMixin):
    ...
    model_name: str = Field(..., alias="model_name")
    api_key: Optional[str] = Field(None, alias="api_key")
    api_protocol: Literal["openai_speech", "dashscope_multimodal"] = Field(
        "openai_speech", alias="api_protocol"
    )
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "api_key": Description(
            en="Optional API key for remote Qwen3 TTS endpoints",
            zh="远程 Qwen3 TTS 接口可选 API 密钥",
        ),
        "api_protocol": Description(
            en="Request protocol: openai_speech (self-hosted) or dashscope_multimodal (Alibaba Cloud)",
            zh="请求协议：openai_speech（自托管）或 dashscope_multimodal（阿里云）",
        ),
        "language": Description(
...
class TTSConfig(I18nMixin):
    ...
    def check_tts_config(self) -> "TTSConfig":
        ...
        elif tts_model == "qwen3_tts" and self.qwen3_tts is not None:
            self.qwen3_tts.model_validate(self.qwen3_tts.model_dump())
